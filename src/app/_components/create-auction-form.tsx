...
export function CreateAuctionForm({ canCreate }: { canCreate: boolean }) {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        ...
        router.push(`/auctions/${createdAuction.id}`);
      } catch (error) {
        setSubmitError(
  ...
  if (!canCreate) {
    return (
      <Card className="rounded-[28px] border-border/90 py-0 shadow-sm">
        <CardHeader className="border-b border-border/80 px-6 py-6">
          <CardTitle className="font-serif text-2xl">Create Auction</CardTitle>
        ...
        <CardContent className="px-6 py-6">
          <Button
            className="h-11 rounded-full bg-foreground px-6 text-base text-background hover:bg-foreground/90"
            nativeButton={false}
